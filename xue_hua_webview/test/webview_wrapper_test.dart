...
void main() {
  test('WebViewController forwards public API calls to the platform', () async {
    ...
    final JavaScriptAsyncResult asyncResult = await controller
        .runJavaScriptAsync(
    ...
    expect(asyncResult.value, 7);
    expect(platform.asyncFunctionBody, 'return await Promise.resolve(value);');
    expect(platform.asyncArguments, <String, Object?>{'value': 7});
    expect(platform.lastAddedUserScript?.source, 'window.injected = true;');
...
class _FakePlatformWebViewController extends PlatformWebViewController {
  ...
  Map<String, Object?>? asyncArguments;
  final List<UserScript> userScripts = <UserScript>[];
  UserScript? lastAddedUserScript;
  ...
  Future<JavaScriptAsyncResult> runJavaScriptAsync(
    ...
    return JavaScriptAsyncResult(value: arguments['value']);
  }

  @override
  Future<void> addUserScript(UserScript userScript) async {
    lastAddedUserScript = userScript;
