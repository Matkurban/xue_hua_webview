...
abstract class PlatformWebViewController extends PlatformInterface {
  ...
  /// Runs [functionBody] as an async JavaScript function and waits for a
  /// returned Promise to settle.
  ///
  /// Keys in [arguments] become local variables inside [functionBody]. Values
  /// must be JSON-serializable.
  ///
  /// When the Promise rejects or the engine throws, the result's
  /// [JavaScriptAsyncResult.error] is set instead of returning an empty object.
  ///
  /// If [timeout] is set and elapses before the Promise settles, [error]
  ...
  Future<JavaScriptAsyncResult> runJavaScriptAsync(
    ...
    throw UnimplementedError(
      'runJavaScriptAsync is not implemented on the current platform',
    );
  }

  /// Injects [userScript] into every subsequent main-frame (and optionally
  /// iframe) navigation.
  ...
  /// Removes every script previously added with [addUserScript].
  ///
  /// Platform implementations must re-apply any internally managed scripts
  /// (JavaScript channels, console forwarding, zoom) after clearing.
  ...
  ///
  /// A disposed controller cannot be reused. Removing a [PlatformWebViewWidget]
  /// does not call this method because a controller can be detached and mounted
  /// again.
  Future<void> dispose() {
    throw UnimplementedError(
