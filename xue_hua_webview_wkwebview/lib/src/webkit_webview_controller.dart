        ...
        _configuration
            ...
            .then(
              ...
              onError: (Object error, StackTrace stackTrace) {
                debugPrint(
                  'xue_hua_webview_wkwebview: failed to configure '
...
class WebKitWebViewController extends PlatformWebViewController {
  ...
  void _logUnsupportedMacOS(
    ...
    final String result = fallback == null
        ...
        : 'Returning $fallback.';
    debugPrint('xue_hua_webview_wkwebview: $method $reason. $result');
  }
  ...
  Future<void> setInspectable(bool inspectable) async {
    ...
    if (defaultTargetPlatform == TargetPlatform.macOS) {
      ...
      debugPrint(
        'xue_hua_webview_wkwebview: setInspectable requires iOS 16.4 or later. '
        'The call was ignored.',
      );
    }
  }
  ...
  WKUserScript _toWkUserScript(UserScript userScript) {
    return WKUserScript(
      source: userScript.source,
      injectionTime: userScript.injectionTime.name == 'documentStart'
  ...
  Map<String, Object> _toWkAsyncArguments(Map<String, Object?> arguments) {
  ...
  Future<JavaScriptAsyncResult> runJavaScriptAsync(
    ...
    Future<JavaScriptAsyncResult> run() async {
      try {
        final Object? result = await _webView.callAsyncJavaScript(
          functionBody,
          _toWkAsyncArguments(arguments),
        );
  ...
  Future<void> addUserScript(UserScript userScript) async {
    ...
    final WKUserContentController contentController = await _webView
        .configuration
        .getUserContentController();
    await contentController.addUserScript(_toWkUserScript(userScript));
  }
  ...
  Future<void> dispose() async {
    ...
    _onPermissionRequestCallback = null;
  ...
  Future<void> _resetUserScripts({String? removedJavaScriptChannel}) async {
    ...
    await Future.wait(<Future<void>>[
      ...
      for (final UserScript userScript in _userScripts)
        controller.addUserScript(_toWkUserScript(userScript)),
    ]);
  }
