...
import 'package:xue_hua_webview_wkwebview/src/common/web_kit.g.dart';
import 'package:xue_hua_webview_wkwebview/src/common/webkit_constants.dart';
...
window.addEventListener("error", function(e) {
  log("error", e.message + " at " + e.filename + ":" + e.lineno + ":" + e.colno);
    ...
    test('addUserScript maps to WKUserScript', () async {
      final mockUserContentController = MockWKUserContentController();
      final WebKitWebViewController controller = createControllerWithMocks(
        mockUserContentController: mockUserContentController,
      );
      ...
      expect(captured.isForMainFrameOnly, isFalse);
    });

    test('runJavaScriptAsync returns native values and errors', () async {
      final mockWebView = MockUIViewWKWebView();
      when(
      ...
      final JavaScriptAsyncResult success = await controller.runJavaScriptAsync(
        'return 1;',
      );
      expect(success.hasError, isFalse);
      expect(success.value, 1);

      final JavaScriptAsyncResult failure = await controller.runJavaScriptAsync(
        'throw new Error("nope");',
      );
      expect(failure.hasError, isTrue);
      expect(failure.error, contains('nope'));
    });
  });
