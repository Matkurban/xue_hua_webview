...
class WebViewProxyAPITests: XCTestCase {
    ...
    func testSafariUserAgentTokenFormat() {
        XCTAssertEqual(
            SafariUserAgent.compatibilityApplicationName(osMajorVersion: 15),
            "Version/18.0 Safari/605.1.15"
        )
        XCTAssertEqual(
            SafariUserAgent.compatibilityApplicationName(osMajorVersion: 26),
            "Version/26.0 Safari/605.1.15"
        )
    }

    func testSafariUserAgentReplacesDefaultBundleName() {
        XCTAssertTrue(
            SafariUserAgent.shouldReplaceApplicationName(
                nil, bundleName: "example", bundleVersion: "1.0.0"
            )
        )
        XCTAssertTrue(
            SafariUserAgent.shouldReplaceApplicationName(
                "example", bundleName: "example", bundleVersion: "1.0.0"
            )
        )
        XCTAssertTrue(
            SafariUserAgent.shouldReplaceApplicationName(
                "example/1.0.0", bundleName: "example", bundleVersion: "1.0.0"
            )
        )
        XCTAssertTrue(
            SafariUserAgent.shouldReplaceApplicationName(
                "example/1.0.0+1", bundleName: "example", bundleVersion: "1.0.0"
            )
        )
        XCTAssertFalse(
            SafariUserAgent.shouldReplaceApplicationName(
                "MyApp/2.0", bundleName: "example", bundleVersion: "1.0.0"
            )
        )
    }

    #if os(macOS)
        @MainActor func testPigeonDefaultConstructorAppliesSafariUserAgent() {
            let registrar = TestProxyApiRegistrar()
            let api = webViewProxyAPI(forRegistrar: registrar)
            ...
            let instance = try? api.pigeonDelegate.pigeonDefaultConstructor(
                pigeonApi: api, initialConfiguration: configuration
            )
            XCTAssertNotNil(instance)
            XCTAssertEqual(
                configuration.applicationNameForUserAgent,
                SafariUserAgent.compatibilityApplicationName(
    ...
    @MainActor func testLoad() throws {
        ...
        let instance = TestViewWKWebView()
        let request = try URLRequestWrapper(URLRequest(url: XCTUnwrap(URL(string: "http://google.com"))))
    ...
    @MainActor func testLoadFlutterAsset() throws {
        let registrar = TestProxyApiRegistrar()
        let api = webViewProxyAPI(forRegistrar: registrar)
        ...
        let url = try XCTUnwrap(try XCTUnwrap(instance.loadFileUrlArgs?[0]))
        let readAccessURL = try XCTUnwrap(try XCTUnwrap(instance.loadFileUrlArgs?[1]))
...
class TestViewWKWebView: WKWebView {
    ...
    var loadArgs: [AnyHashable?]?
    var loadHtmlStringArgs: [AnyHashable?]?
    var loadFileUrlArgs: [URL]?
    var goBackCalled = false
    var goForwardCalled = false
    var reloadCalled = false
    var setAllowsBackForwardNavigationGesturesArgs: [AnyHashable?]?
    var setCustomUserAgentArgs: [AnyHashable?]?
    var evaluateJavaScriptArgs: [AnyHashable?]?
    var setInspectableArgs: [AnyHashable?]?
