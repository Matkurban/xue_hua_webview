...
extension WKFrameInfo {
    ...
    var maybeRequest: URLRequest? {
        return perform(#selector(getter: WKFrameInfo.request))?.takeUnretainedValue()
            as! URLRequest?
    }
}
...
class FrameInfoProxyAPIDelegate: PigeonApiDelegateWKFrameInfo {
    func isMainFrame(pigeonApi _: PigeonApiWKFrameInfo, pigeonInstance: WKFrameInfo) throws -> Bool {
        return pigeonInstance.isMainFrame
    }

    func request(pigeonApi _: PigeonApiWKFrameInfo, pigeonInstance: WKFrameInfo) throws
        -> URLRequestWrapper?
    {
