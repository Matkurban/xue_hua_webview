...
public class NavigationDelegateImpl: NSObject, WKNavigationDelegate {
    ...
    public func webView(
        _ webView: WKWebView, didFail _: WKNavigation!, withError error: Error
    ) {
        registrar.dispatchOnMainThread { onFailure in
            ...
            self.api.decidePolicyForNavigationAction(
                ...
                DispatchQueue.main.async {
                    ...
                    case let .success(policy):
                        self.finishNavigationActionDecision(
                            self.nativeNavigationActionPolicy(policy),
                            for: navigationAction,
                            decisionHandler: decisionHandler
                        )
                    case let .failure(error):
                        if ExternalUrlCoordinator.isExternal(navigationAction.request.url) {
                            ExternalUrlCoordinator.open(navigationAction.request.url)
    ...
    private func finishNavigationActionDecision(
        _ policy: WKNavigationActionPolicy,
        for navigationAction: WKNavigationAction,
        decisionHandler: @escaping (WKNavigationActionPolicy) -> Void
    ) {
        let url = navigationAction.request.url
        if ExternalUrlCoordinator.isExternal(url) {
            if policy == .allow {
                ExternalUrlCoordinator.open(url)
            }
            decisionHandler(.cancel)
            return
        }
...
class NavigationDelegateProxyAPIDelegate: PigeonApiDelegateWKNavigationDelegate {
    ...
    func setChallengeHandling(
        pigeonApi _: PigeonApiWKNavigationDelegate, pigeonInstance: WKNavigationDelegate,
