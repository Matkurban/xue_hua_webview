...
import 'package:xue_hua_webview_linux/src/linux_webview_constants.dart';
...
void main() {
  ...
  test('creates Linux platform implementation objects', () {
    ...
    expect(
      ...
      isA<LinuxWebViewStorageManager>(),
    );
    expect(
      platform.createPlatformHeadlessWebView(
        const PlatformHeadlessWebViewCreationParams(),
      ),
      isA<LinuxHeadlessWebView>(),
    );
  });

  test('addUserScript and runJavaScriptAsync use native methods', () async {
    ...
    final JavaScriptAsyncResult result = await controller.runJavaScriptAsync(
      'return 1;',
      arguments: const <String, Object?>{'x': 1},
    );

    expect(
      calls.map((MethodCall call) => call.method),
      containsAll(<String>['addUserScript', 'runJavaScriptAsync']),
    );
