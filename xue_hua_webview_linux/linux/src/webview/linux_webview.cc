...
constexpr const gchar *kResourceRequestDetailsKey =
    "xue_hua_webview_linux_request_details";
...
void destroy_linux_webview(gpointer data) {
  ...
  if (webview->web_view != nullptr) {
    ...
    g_object_set_data(G_OBJECT(webview->web_view), kLinuxWebViewInstanceKey,
                      nullptr);
    if (webview->offscreen_host != nullptr) {
      gtk_widget_destroy(webview->offscreen_host);
      webview->offscreen_host = nullptr;
    } else {
  ...
  if (webview->user_scripts != nullptr) {
    g_ptr_array_unref(webview->user_scripts);
  }
  g_free(webview);
}
...
LinuxWebView *create_linux_webview(XueHuaWebviewLinuxPlugin *self) {
  ...
  webview->js_channels =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, nullptr);
  webview->user_scripts =
      g_ptr_array_new_with_free_func(destroy_linux_user_script);
  ...
  gchar *method_name =
      g_strdup_printf("com.kurban.xue_hua_webview_linux/%d", webview->id);
  ...
  gchar *event_name =
      g_strdup_printf("com.kurban.xue_hua_webview_linux/%d/events", webview->id);
  ...
  webkit_user_content_manager_register_script_message_handler(
      webview->content_manager, "__xue_hua_webview_console");
  webkit_user_content_manager_register_script_message_handler(
      webview->content_manager, "__xue_hua_webview_scroll");
  g_signal_connect(webview->content_manager,
                   "script-message-received::__xue_hua_webview_console",
                   G_CALLBACK(console_message_received_cb), webview);
  g_signal_connect(webview->content_manager,
                   "script-message-received::__xue_hua_webview_scroll",
  ...
  if (overlay != nullptr) {
    ...
    webview->offscreen_host = gtk_offscreen_window_new();
    gtk_widget_set_size_request(webview->offscreen_host, 800, 600);
    gtk_container_add(GTK_CONTAINER(webview->offscreen_host),
                      GTK_WIDGET(webview->web_view));
    gtk_widget_show_all(webview->offscreen_host);
  }
