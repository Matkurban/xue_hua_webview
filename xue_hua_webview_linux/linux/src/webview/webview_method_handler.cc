...
void instance_method_call_cb(FlMethodChannel *channel,
  ...
  if (strcmp(method, "runJavaScriptAsync") == 0) {
    const gchar *function_body = map_lookup_string(args, "functionBody");
    if (function_body == nullptr) {
      respond(method_call, error_response("invalid_script",
                                          "functionBody is required."));
      return;
    }
    call_async_javascript(webview->web_view, function_body,
                          map_lookup(args, "arguments"), method_call);
    return;
  }
  ...
  if (strcmp(method, "addUserScript") == 0) {
    ...
    if (source == nullptr || strlen(source) == 0) {
      respond(method_call,
              error_response("invalid_script", "UserScript source is required."));
      return;
    }
    LinuxUserScript *entry = g_new0(LinuxUserScript, 1);
    entry->source = g_strdup(source);
    entry->at_document_start = map_lookup_bool(args, "atDocumentStart", TRUE);
    entry->for_main_frame_only =
        map_lookup_bool(args, "forMainFrameOnly", TRUE);
    g_ptr_array_add(webview->user_scripts, entry);
  ...
  if (strcmp(method, "removeAllUserScripts") == 0) {
    g_ptr_array_set_size(webview->user_scripts, 0);
    rebuild_user_scripts(webview);
    respond(method_call, success_response());
    return;
  }
  ...
  if (strcmp(method, "dispose") == 0) {
    ...
    schedule_flutter_view_input_region_update(webview->plugin);
    XueHuaWebviewLinuxPlugin *plugin = webview->plugin;
