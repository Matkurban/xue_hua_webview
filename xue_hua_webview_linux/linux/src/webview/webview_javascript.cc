...
static GVariant *fl_value_to_variant(FlValue *value);

static GVariant *fl_value_to_variant(FlValue *value) {
  if (value == nullptr) {
    return g_variant_new_maybe(G_VARIANT_TYPE_BOOLEAN, nullptr);
  }
  ...
  case FL_VALUE_TYPE_INT:
    return g_variant_new_int64(fl_value_get_int(value));
  ...
  case FL_VALUE_TYPE_LIST: {
    ...
    for (size_t i = 0; i < length; i++) {
      g_variant_builder_add(&builder, "v",
                            fl_value_to_variant(fl_value_get_list_value(value, i)));
    }
  ...
  case FL_VALUE_TYPE_MAP: {
    ...
    for (size_t i = 0; i < length; i++) {
      ...
      FlValue *item = fl_value_get_map_value(value, i);
      if (key == nullptr || fl_value_get_type(key) != FL_VALUE_TYPE_STRING) {
        continue;
      }
      g_variant_builder_add(&builder, "{sv}", fl_value_get_string(key),
                            fl_value_to_variant(item));
    }
    return g_variant_builder_end(&builder);
  }
  default:
...
static void async_javascript_finished_cb(GObject *object, GAsyncResult *result,
                                         gpointer user_data) {
  FlMethodCall *method_call = FL_METHOD_CALL(user_data);
  GError *error = nullptr;
...
#if WEBKIT_CHECK_VERSION(2, 40, 0)
  GVariant *gargs = nullptr;
  if (arguments != nullptr && fl_value_get_type(arguments) == FL_VALUE_TYPE_MAP &&
      fl_value_get_length(arguments) > 0) {
    gargs = fl_value_to_variant(arguments);
  }
  g_object_ref(method_call);
  webkit_web_view_call_async_javascript_function(
      web_view, function_body, gargs, nullptr, nullptr, nullptr,
      async_javascript_finished_cb, method_call);
#else
  (void)web_view;
  (void)function_body;
  (void)arguments;
  respond(method_call,
          error_response("unsupported",
                         "call_async_javascript_function requires WebKitGTK "
                         "2.40 or newer."));
#endif
}
...
void destroy_linux_user_script(gpointer data) {
  LinuxUserScript *script = static_cast<LinuxUserScript *>(data);
  if (script == nullptr) {
    return;
  }
  g_free(script->source);
  g_free(script);
}
...
gchar *build_scrollbar_style_script(LinuxWebView *webview) {
  ...
  gchar *script = g_strdup_printf(R"(
    (function() {
      const styleId = '__xue_hua_webview_scrollbars';
      const css = "%s";
...
gchar *build_overscroll_style_script(LinuxWebView *webview) {
  ...
  gchar *script = g_strdup_printf(R"(
    (function() {
      const styleId = '__xue_hua_webview_overscroll';
      const value = "%s";
      let style = document.getElementById(styleId);
      if (!value) {
        if (style) {
...
void rebuild_user_scripts(LinuxWebView *webview) {
    ...
    (function() {
      ...
      ['log', 'info', 'warn', 'error', 'debug'].forEach(function(level) {
        ...
        console[level] = function() {
          try {
            window.webkit.messageHandlers.__xue_hua_webview_console.postMessage(
  ...
  add_user_script(webview->content_manager, R"(
    (function() {
      ...
      window.addEventListener('scroll', function() {
        try {
          window.webkit.messageHandlers.__xue_hua_webview_scroll.postMessage(
  ...
  if (webview->user_scripts != nullptr) {
    for (guint i = 0; i < webview->user_scripts->len; i++) {
      LinuxUserScript *entry = static_cast<LinuxUserScript *>(
          g_ptr_array_index(webview->user_scripts, i));
      if (entry == nullptr || entry->source == nullptr) {
        continue;
      }
      WebKitUserScript *script = webkit_user_script_new(
          entry->source,
          entry->for_main_frame_only ? WEBKIT_USER_CONTENT_INJECT_TOP_FRAME
                                     : WEBKIT_USER_CONTENT_INJECT_ALL_FRAMES,
          entry->at_document_start ? WEBKIT_USER_SCRIPT_INJECT_AT_DOCUMENT_START
