...
constexpr const gchar* kFlutterViewOverlayKey = "xue_hua_webview_linux_overlay";
constexpr const gchar *kFlutterInputWidgetKey =
    "xue_hua_webview_linux_flutter_input_widget";
...
gboolean flutter_view_parent_set_emission_hook(
  ...
  if (gtk_widget_get_realized(view_widget)) {
    g_warning(
        "xue_hua_webview_linux could not install its GtkOverlay before FlView "
        "was realized.");
    return TRUE;
  }
  ...
  if (children == nullptr || children->next != nullptr) {
    g_warning(
        "xue_hua_webview_linux expected an unrealized FlView with one child.");
    g_list_free(children);
    return TRUE;
  }
...
static void flutter_view_size_allocate_cb(GtkWidget* widget,
  ...
  schedule_flutter_view_input_region_update(
      static_cast<XueHuaWebviewLinuxPlugin *>(user_data));
}
...
static void overlay_destroy_cb(GtkWidget *widget, gpointer user_data) {
  detach_linux_webview_host(static_cast<XueHuaWebviewLinuxPlugin *>(user_data));
}
...
static void attach_linux_webview_host(XueHuaWebviewLinuxPlugin *self,
  ...
  if (self->flutter_input_widget == nullptr) {
    g_warning(
        "xue_hua_webview_linux could not locate Flutter's GTK input widget; "
...
GtkOverlay* ensure_overlay(XueHuaWebviewLinuxPlugin* self) {
  ...
  g_warning(
      "xue_hua_webview_linux requires its automatic GtkOverlay to be installed "
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    if (!gtk_widget_translate_coordinates(GTK_WIDGET(webview->web_view),
      ...
      if (!self->input_region_warning_emitted) {
        g_warning(
            "xue_hua_webview_linux could not translate the WebView input region "
