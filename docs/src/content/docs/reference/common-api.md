---
title: Common API
description: Complete app-facing API reference for xue_hua_webview.
---
...
| `runJavaScriptAsync(String functionBody, {Map<String, Object?> arguments, Duration? timeout})` | `Future<JavaScriptAsyncResult>` |
| `addUserScript(UserScript userScript)` | `Future<void>` |
...
Clears website data without a live `WebViewController`.

| Method | Return | Use |
| --- | --- | --- |
| `WebViewStorageManager()` | constructor | Creates the current platform storage manager. |
| `removeData({Set<WebViewDataType> dataTypes, DateTime? since})` | `Future<void>` | Clears cookies, HTTP cache, and DOM storage. |

`WebViewDataType` values are `cookies`, `httpCache`, `localStorage`, `sessionStorage`, `indexedDb`, `webSql`, and `all`. Web only clears the host origin.

## `HeadlessWebView`

Runs a WebView without inserting `WebViewWidget`.

| Member | Use |
| --- | --- |
| `HeadlessWebView()` | Creates an offscreen WebView and controller. |
| `controller` | The wrapped `WebViewController`. |
| `run()` | Starts the runtime so it can load content. |
| `isRunning` | True after `run()` and before `dispose()`. |
| `dispose()` | Releases native resources. Also available on `WebViewController`. |

## Exported Platform Interface Types

`xue_hua_webview.dart` exports the platform interface types needed by app code, including:
...
- `WebViewDataType`
- platform creation params for controller, delegate, widget, cookie manager, storage manager, and headless WebView
