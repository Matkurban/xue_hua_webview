...
`canGoBack` and `canGoForward` reflect the platform history state. On web, `xue_hua_webview_web` maintains a logical history for loads initiated through the controller.
...
Await a Promise with named arguments:

```dart
final JavaScriptAsyncResult asyncResult = await controller.runJavaScriptAsync(
  'return await Promise.resolve(value);',
  arguments: const <String, Object?>{'value': 2},
);

await controller.addUserScript(
  const UserScript(source: 'window.__injected = true;'),
);
```
...
Scrollbar rendering remains engine-specific. macOS
