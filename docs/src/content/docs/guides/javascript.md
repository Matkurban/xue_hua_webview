...
Android, iOS, macOS, Windows, Linux, and controllable Web content
...
- `runJavaScriptAsync`
- `addUserScript` (srcdoc and same-origin only)
...
`functionBody` runs inside an async function. Named [arguments] become JS
locals. iOS 14+ / macOS 11+ use native `callAsyncJavaScript`. Linux uses
`webkit_web_view_call_async_javascript_function` when WebKitGTK is 2.40 or
newer. Android, Windows, and older Linux inject a document-start helper.
Web awaits Promises on same-origin or plugin-managed isolated HTML.
...
Document-start injection is native on WKWebView, WebKitGTK, WebView2, and
Android WebView 91+. Older Android falls back to `onPageStarted`. Web injects
script tags into srcdoc/managed HTML; cross-origin iframes cannot be patched.
