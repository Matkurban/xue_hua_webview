...
Linux is provided by `xue_hua_webview_linux 1.0.0` and uses WebKitGTK.
...
`xue_hua_webview_linux` does not claim passkey support or expose a setting that the
...
- `addUserScript` uses `webkit_user_content_manager_add_script`. Internal
  console, scroll, and channel scripts are restored after
  `removeAllUserScripts`.
- `runJavaScriptAsync` uses `webkit_web_view_call_async_javascript_function`
  on WebKitGTK 2.40+ and falls back to the helper channel on older versions.
- `WebViewStorageManager` uses `webkit_website_data_manager_clear` on the
  default WebContext.
- Headless mode hosts `WebKitWebView` in an offscreen `GtkWindow` when no
  Flutter overlay is attached.
