...
Web is provided by `xue_hua_webview_web 1.0.2` and renders an HTML `iframe`.
...
| `addUserScript` / `runJavaScriptAsync` | Same-origin or plugin-managed isolated HTML only. |
| `WebViewStorageManager` | Clears host-origin cookies, Cache Storage, local/session storage, and IndexedDB. |
| `HeadlessWebView` | Creates a hidden iframe on `document.body` and removes it on dispose. |
