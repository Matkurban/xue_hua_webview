...
Windows 由 `xue_hua_webview_windows 1.0.3` 提供，底层使用 Microsoft Edge WebView2。
...
凭据提供方处理；`xue_hua_webview` 不会关闭或替换这条通路。
...
- surface resize 使用代际校验并跟随运行中的显示器和 DPI 变化，旧任务不会
  覆盖当前 texture 尺寸。
- document-start `UserScript` 使用 `addScriptToExecuteOnDocumentCreated`；
  document-end 在导航完成后注入。
- `runJavaScriptAsync` 通过内部 helper 和 WebMessage 回传结果。
- `WebViewStorageManager` 必要时创建临时隐藏 WebView2 environment，完成后释放。
- 无头 WebView2 无需 texture 消费者即可运行，`dispose()` 释放原生资源。
