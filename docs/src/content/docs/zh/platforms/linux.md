...
Linux 由 `xue_hua_webview_linux 1.0.2` 提供，底层使用 WebKitGTK。
...
`xue_hua_webview_linux` 不声明支持 Passkey，也不暴露底层无法执行的虚假开关。
...
- `addUserScript` 使用 `webkit_user_content_manager_add_script`；
  `removeAllUserScripts` 后会重放内部 console、滚动和 channel 脚本。
- `runJavaScriptAsync` 在 WebKitGTK 2.40+ 使用原生
  `webkit_web_view_call_async_javascript_function`，旧版本走内部通道。
- `WebViewStorageManager` 使用默认 WebContext 的
  `webkit_website_data_manager_clear`。
- 无头模式在没有 Flutter overlay 时把 `WebKitWebView` 放进 offscreen
  `GtkWindow`。
