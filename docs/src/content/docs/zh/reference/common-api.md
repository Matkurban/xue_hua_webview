---
title: 通用接口
description: xue_hua_webview 应用侧 API 参考。
---
...
| `runJavaScriptAsync` | `Future<JavaScriptAsyncResult>` |
| `addUserScript` / `removeAllUserScripts` | `Future<void>` |
...
无需活 `WebViewController` 即可清理网站数据。`removeData({dataTypes, since})` 可清理 cookie、HTTP cache 和 DOM storage。Web 仅限宿主 origin。

## `HeadlessWebView`

不插入 `WebViewWidget` 也能加载页面、执行 JS 并接收回调。使用 `run()` / `dispose()` / `isRunning`，通过 `controller` 操作。

## 导出的接口类型

`xue_hua_webview.dart` 导出 `LoadRequestMethod`、`NavigationDecision`、`NavigationRequest`、`WebResourceError`、`HttpResponseError`、`HttpAuthRequest`、`WebViewCredential`、`JavaScriptMode`、`JavaScriptMessage`、`JavaScriptConsoleMessage`、JS dialog request、`ScrollPositionChange`、`WebViewCookie`、`WebViewOverScrollMode`、`WebViewPermissionResourceType`、`WebViewPlatform`、`UserScript`、`UserScriptInjectionTime`、`JavaScriptAsyncResult` 和 `WebViewDataType` 等类型。
