...
class WindowsWebViewController extends PlatformWebViewController
  ...
  final Map<UserScript, String> _userScriptIds = <UserScript, String>{};
  ...
  static const String _channelMessageType = '__windows_xue_hua_webview_type';
  ...
  Future<T> _awaitPendingRequest<T>(
    ...
    try {
      return await decision().timeout(
        ...
        onTimeout: () {
          debugPrint(
            'xue_hua_webview_windows: $requestName did not complete within '
      ...
      debugPrint(
        'xue_hua_webview_windows: $requestName failed; the safe default was used: '
  ...
  void _handleLoadingStateChanged(native_types.LoadingState state) {
    switch (state) {
      ...
      case native_types.LoadingState.navigationCompleted:
        ...
        unawaited(_injectDocumentEndScripts());
  ...
  Future<Map<String, Object?>?> _handleHttpAuthRequested(
    ...
    try {
      ...
      if (completer.isCompleted) {
        debugPrint(
          'xue_hua_webview_windows: HTTP authentication callback failed after '
      ...
      debugPrint(
        'xue_hua_webview_windows: HTTP authentication callback failed; the request '
  ...
  Future<Map<String, Object?>?> _handleSslAuthError(
    ...
    try {
      ...
      if (completer.isCompleted) {
        debugPrint(
          'xue_hua_webview_windows: SSL authentication callback failed after '
          'completing its decision; the completed decision was preserved: '
          '${_singleLineWindowsLogValue(error)}',
        );
        return completer.future;
      ...
      debugPrint(
        'xue_hua_webview_windows: SSL authentication callback failed; the request '
        'was canceled: ${_singleLineWindowsLogValue(error)}',
      );
      return <String, Object?>{'action': 'cancel'};
    }
  ...
  Future<void> setPlatformNavigationDelegate(
    ...
    final Future<void> Function(bool enabled) listener = (bool enabled) async {
      ...
      try {
        ...
        debugPrint(
          'xue_hua_webview_windows: failed to update navigation request '
  ...
  Future<void> addUserScript(UserScript userScript) async {
    ...
    if (userScript.injectionTime == UserScriptInjectionTime.documentStart) {
      final String? scriptId = await _webviewController
          .addScriptToExecuteOnDocumentCreated(userScript.source);
      if (scriptId != null) {
        _userScriptIds[userScript] = scriptId;
      }
    }
  }
  ...
  Future<void> removeAllUserScripts() async {
    await _ensureInitialized();
    for (final String scriptId in _userScriptIds.values) {
  ...
  Future<void> _injectDocumentEndScripts() async {
    ...
    _webviewController.setPermissionRequestedDelegate((
      ...
      try {
        ...
        if (request.decision.isCompleted) {
          debugPrint(
            'xue_hua_webview_windows: permission callback failed after completing '
        ...
        debugPrint(
          'xue_hua_webview_windows: permission callback failed; the safe default '
  ...
  Future<void> _executeScriptBestEffort(String operation, String script) async {
    try {
      ...
      debugPrint(
        'xue_hua_webview_windows: failed to apply $operation to the current '
  ...
  String _scrollBarStyleScript() {
    ...
    return '''
      (function() {
        const styleId = '__flutter_xue_hua_webview_scrollbars';
  ...
  String _overScrollStyleScript() {
    ...
    return '''
      (function() {
        const styleId = '__flutter_xue_hua_webview_overscroll';
...
class _WindowsWebViewState extends State<_WindowsWebView> {
  ...
  Future<void> _openWebView2DownloadPage() async {
    try {
      ...
      debugPrint(
        'xue_hua_webview_windows: failed to open the WebView2 download page: $error',
      );
    }
  }
