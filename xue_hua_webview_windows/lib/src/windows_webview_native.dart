  ...
  Future<void> _initialize() async {
    try {
      ...
      try {
        ...
        debugPrint(
          'xue_hua_webview_windows: failed to clean up after a WebView2 initialization error: ${_singleLineNativeLogValue(cleanupError)}',
        );
      } finally {
  ...
  Future<void> _updateSurfaceAttachment(int generation, bool attached) async {
    try {
      ...
      if (!_isDisposed && generation == _surfaceAttachmentGeneration) {
        debugPrint(
          'xue_hua_webview_windows: failed to update the WebView2 surface attachment: ${_singleLineNativeLogValue(error)}',
        );
      }
    }
  }
...
class _WebviewState extends State<Webview> with WidgetsBindingObserver {
  ...
  Future<void> _reportSurfaceSize() async {
    ...
    try {
      ...
      if (mounted) {
        debugPrint(
          'xue_hua_webview_windows: failed to update the WebView2 surface size: ${_singleLineNativeLogValue(error)}',
        );
      }
    }
  }
