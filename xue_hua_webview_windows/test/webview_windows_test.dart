...
import 'package:xue_hua_webview_windows/src/windows_webview_api.g.dart';
import 'package:xue_hua_webview_windows/src/windows_webview_constants.dart';
import 'package:xue_hua_webview_windows/src/windows_webview_native.dart'
    as native_webview;
import 'package:xue_hua_webview_windows/src/windows_webview_types.dart'
    as native_types;
...
void main() {
  ...
  test('creates Windows platform implementation objects', () {
    ...
    expect(
      platform.createPlatformStorageManager(
        const PlatformWebViewStorageManagerCreationParams(),
      ),
      isA<WindowsWebViewStorageManager>(),
    );
    expect(
      platform.createPlatformHeadlessWebView(
        const PlatformHeadlessWebViewCreationParams(),
      ),
      isA<WindowsHeadlessWebView>(),
    );
  });

  test('addUserScript registers a document-created script', () async {
    ...
    await controller.addUserScript(
      const UserScript(source: 'window.ready = true;'),
    );

    expect(scripts, contains('window.ready = true;'));
  });
    ...
    () async {
      ...
      await _emitWindowsWebViewEvent(<String, Object?>{
        ...
        'value': jsonEncode(<String, Object?>{
          '__windows_xue_hua_webview_type': 'consoleMessage',
          'level': 'warning',
          'message': 'careful',
        }),
      });
      ...
      await _emitWindowsWebViewEvent(<String, Object?>{
        'type': 'webMessageReceived',
        'value': jsonEncode(<String, Object?>{
          '__windows_xue_hua_webview_type': 'scrollPositionChange',
          'x': 12.5,
          'y': 34,
        }),
      });
  ...
  test('clears managed local mappings before remote navigation', () async {
    ...
    final Directory tempDir = Directory.systemTemp.createTempSync(
      'xue_hua_webview_windows_mapping_test_',
    );
...
String _hostApiChannel(String method) =>
    'dev.flutter.pigeon.xue_hua_webview_windows.WindowsWebViewHostApi.$method';
