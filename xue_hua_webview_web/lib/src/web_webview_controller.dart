...
class WebWebViewController extends PlatformWebViewController {
  ...
  WebWebViewController(PlatformWebViewControllerCreationParams params)
    ...
    _loadEventSubscription = _webWebViewParams.iFrame.onLoad.listen((_) {
      ...
      target._injectUserScriptsBestEffort();
  ...
  static const String _scrollBarStyleId = '__xue_hua_webview_scrollbars';
  static const String _channelMessageType = '__xue_hua_webview_type';
  ...
  final List<UserScript> _userScripts = <UserScript>[];
  ...
  void _logLimitationOnce(String key, String message) {
    if (_reportedLimitations.add(key)) {
      debugPrint('xue_hua_webview_web: $message');
    }
  }
  ...
  void _runAsyncCallbackSafely(
    ...
    try {
      ...
      debugPrint(
        'xue_hua_webview_web: $callbackName failed: '
  ...
  String _prepareInlineHtml(String html) {
    String prepared = _injectUserScriptsIntoHtml(html);
  ...
  String _injectUserScriptsIntoHtml(String html) {
    String prepared = html;
    final StringBuffer startScripts = StringBuffer();
    final StringBuffer endScripts = StringBuffer();
    for (final UserScript userScript in _userScripts) {
      final String tag = _userScriptTag(userScript);
      if (userScript.injectionTime == UserScriptInjectionTime.documentStart) {
        startScripts.write(tag);
      } else {
        endScripts.write(tag);
      }
    }
    if (startScripts.isNotEmpty) {
      prepared = _injectIntoDocumentHead(prepared, startScripts.toString());
    }
    if (endScripts.isNotEmpty) {
      prepared = _injectBeforeBodyEnd(prepared, endScripts.toString());
    }
    return prepared;
  }

  String _userScriptTag(UserScript userScript) {
    final String escaped = userScript.source.replaceAll(
      '</script',
      r'<\/script',
    );
  ...
  void _installScriptBestEffort(String feature, String script) {
    unawaited(
      _evaluateScriptInCurrentDocument(script).then<void>(
        ...
        onError: (Object error, StackTrace stackTrace) {
          debugPrint(
            'xue_hua_webview_web: failed to install $feature in the current '
            'document: $error',
          );
        },
      ),
  ...
  @override
  Future<JavaScriptAsyncResult> runJavaScriptAsync(
    String functionBody, {
    ...
    for (final String name in arguments.keys) {
      if (!_javaScriptIdentifierPattern.hasMatch(name)) {
        throw ArgumentError.value(
          name,
          'arguments',
          'JavaScript async argument names must be valid identifiers.',
        );
      }
    }

    Future<JavaScriptAsyncResult> run() async {
      final web.Window? window = _tryReadAccessibleContentWindow();
      if (window != null) {
        return _runJavaScriptAsyncOnWindow(window, functionBody, arguments);
      }
      if (_hasIsolatedBridge) {
        try {
          final Object? result = await _invokeIsolatedBridge(
          ...
          return JavaScriptAsyncResult(value: result);
        } on Object catch (error) {
          return JavaScriptAsyncResult(error: error.toString());
        }
      }
      return const JavaScriptAsyncResult(
        error:
            'Running async JavaScript is only supported for same-origin or srcdoc iframe content.',
      );
    }
    ...
    return run().timeout(
      timeout,
      onTimeout: () => JavaScriptAsyncResult(
        error:
  ...
  Future<JavaScriptAsyncResult> _runJavaScriptAsyncOnWindow(
    web.Window window,
    String functionBody,
    ...
    for (final MapEntry<String, Object?> entry in arguments.entries) {
      declarations.writeln('const ${entry.key} = ${jsonEncode(entry.value)};');
    }
    final String script =
        '''
(async function() {
...
''';
    try {
      final JSAny? result = window.evaluateJavaScript(script);
      if (result == null) {
        return const JavaScriptAsyncResult();
      }
      if (result.isA<JSPromise>()) {
        final JSPromise<JSAny?> promise = result as JSPromise<JSAny?>;
        final JSAny? settled = await promise.toDart;
        if (settled == null) {
          return const JavaScriptAsyncResult();
        }
        return JavaScriptAsyncResult(
          value: _nullableDartObjectFromJavaScriptResult(settled),
        );
      }
      return JavaScriptAsyncResult(
        value: _nullableDartObjectFromJavaScriptResult(result),
      );
    } on Object catch (error) {
      return JavaScriptAsyncResult(error: error.toString());
    }
  ...
  Object? _nullableDartObjectFromJavaScriptResult(JSAny result) {
    try {
      final JSString? jsonResult = _jsonStringify(result);
      if (jsonResult == null) {
        return null;
      }
      return jsonDecode(jsonResult.toDart);
    } on Object {
  ...
  Future<void> addUserScript(UserScript userScript) async {
    ...
    if (userScript.injectionTime == UserScriptInjectionTime.documentEnd) {
      _injectUserScriptsBestEffort();
    }
  }
  ...
  void _injectUserScriptsBestEffort() {
    if (_javaScriptMode == JavaScriptMode.disabled) {
      return;
    }
    if (_hasIsolatedBridge || _scriptsAlreadyInCurrentDocument) {
      return;
    }
    for (final UserScript userScript in _userScripts) {
      _installScriptBestEffort('user script', userScript.source);
    }
  }

  bool get _scriptsAlreadyInCurrentDocument =>
      _activeHistoryEntry is _HtmlHistoryEntry ||
      (_activeHistoryEntry is _XhrHistoryEntry &&
          _isHtmlMimeType((_activeHistoryEntry as _XhrHistoryEntry).mimeType));

  void attachOffscreen() {
    final web.HTMLIFrameElement iFrame = _webWebViewParams.iFrame;
    iFrame.style
  ...
  Future<void> dispose() {
    ...
    return _nativeDisposal.dispose();
  }
      ...
      case _platformPermissionRequestMessageType:
        ...
        try {
          ...
          if (request._isCompleted) {
            debugPrint(
              'xue_hua_webview_web: platform permission callback failed after '
              'completing its decision; the completed decision was preserved: '
              '$failure',
            );
          } else {
            debugPrint(
              'xue_hua_webview_web: platform permission callback failed; the '
  ...
  void _logJavaScriptDialogCallbackFailure(
    ...
    debugPrint(
      'xue_hua_webview_web: JavaScript $dialogName callback failed: '
      '${error.toString().replaceAll(RegExp(r'[\r\n]+'), ' ')}; '
  ...
  String _decodeResponseBody(Encoding encoding, Uint8List bytes) {
    try {
      ...
      debugPrint(
        'xue_hua_webview_web: response bytes were invalid for '
  ...
  String _injectBeforeBodyEnd(String html, String content) {
    final Match? match = RegExp(
      r'</body>',
      caseSensitive: false,
    ).firstMatch(html);
    if (match != null) {
      return html.replaceRange(match.start, match.start, content);
    }
    return '$html$content';
  }
  ...
  window.addEventListener('message', function(event) {
    ...
    try {
      switch (data.action) {
        ...
        case 'evaluateAsync': {
          const functionBody = String(payload.functionBody || '');
          const args = payload.arguments || {};
          const names = Object.keys(args);
          const values = names.map(function(name) { return args[name]; });
          const asyncFn = Function.apply(
            null,
            names.concat(['return (async function() {\\n' + functionBody + '\\n})();'])
          );
          Promise.resolve(asyncFn.apply(null, values)).then(function(value) {
            const encoded = JSON.stringify(value);
            respond(requestId, encoded === undefined ? null : JSON.parse(encoded));
          }, function(error) {
...
''';
  ...
  void _activateXhrEntry(
    ...
    if (_isHtmlMimeType(entry.mimeType)) {
      ...
      final String withScripts = _injectUserScriptsIntoHtml(responseBody);
