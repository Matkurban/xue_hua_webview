...
public class WebViewProxyApi extends PigeonApiWebView {
  private final Map<String, ScriptHandler> documentStartScripts = new HashMap<>();
  ...
  @NonNull
  @Override
  public String addDocumentStartJavaScript(
      @NonNull WebView pigeon_instance,
      @NonNull String script,
      @NonNull List<String> allowedOriginRules) {
    if (!WebViewFeature.isFeatureSupported(WebViewFeature.DOCUMENT_START_SCRIPT)) {
      throw new UnsupportedOperationException(
          "DOCUMENT_START_SCRIPT is not supported by the installed WebView.");
    }
    final ScriptHandler handler =
        WebViewCompat.addDocumentStartJavaScript(
            pigeon_instance, script, new HashSet<>(allowedOriginRules));
    final String scriptId = UUID.randomUUID().toString();
    documentStartScripts.put(scriptId, handler);
    return scriptId;
  }

  @Override
  public void removeDocumentStartJavaScript(
      @NonNull WebView pigeon_instance, @NonNull String scriptId) {
    final ScriptHandler handler = documentStartScripts.remove(scriptId);
    if (handler != null) {
      handler.remove();
    }
  }
  ...
  public void clearHttpCache() {
    final WebView webView = new WebView(getPigeonRegistrar().getContext());
