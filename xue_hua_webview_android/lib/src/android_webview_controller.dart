...
void _reportAndroidCallbackError(String callbackName, Object error) {
  debugPrint(
    'xue_hua_webview_android: $callbackName failed: '
...
class AndroidWebViewController extends PlatformWebViewController
    with JavaScriptAsyncBridgeMixin {
  ...
  final Map<UserScript, String> _documentStartHandles = <UserScript, String>{};
  bool? _documentStartSupported;
  bool _userScriptClientInstalled = false;
  ...
  Future<void> setPlatformNavigationDelegate(
    ...
    _installUserScriptHooks(handler);
    await Future.wait(<Future<void>>[
  ...
  void _installUserScriptHooks(AndroidNavigationDelegate handler) {
    handler.setInternalPageCallbacks(
      onPageStarted: _injectFallbackDocumentStartScripts,
      onPageFinished: _injectDocumentEndScripts,
    );
  }

  void _ensureUserScriptPageClient() {
    if (_currentNavigationDelegate != null || _userScriptClientInstalled) {
      return;
    }
    _userScriptClientInstalled = true;
    unawaited(
      _webView.setWebViewClient(
        android_webview.WebViewClient(
          onPageStarted: (_, __, String url) {
            _injectFallbackDocumentStartScripts(url);
          },
          onPageFinished: (_, __, String url) {
            _injectDocumentEndScripts(url);
          },
        ),
      ),
    );
  ...
  Future<void> addUserScript(UserScript userScript) async {
    ...
    if (userScript.injectionTime == UserScriptInjectionTime.documentStart) {
      await _tryAddDocumentStartScript(userScript);
    }
    _ensureUserScriptPageClient();
  }

  @override
  Future<void> removeAllUserScripts() async {
    for (final String scriptId in _documentStartHandles.values) {
      await _webView.removeDocumentStartJavaScript(scriptId);
    }
    _documentStartHandles.clear();
    _userScripts.clear();
  }

  @override
  Future<void> dispose() => _webView.destroy();

  Future<void> _tryAddDocumentStartScript(UserScript userScript) async {
    _documentStartSupported ??=
        await android_webview.WebViewFeature.isFeatureSupported(
          WebViewFeatureConstants.documentStartScript,
        );
    if (!(_documentStartSupported ?? false)) {
      return;
    }
    try {
      final String scriptId = await _webView.addDocumentStartJavaScript(
        userScript.source,
        const <String>['*'],
      );
      _documentStartHandles[userScript] = scriptId;
    } on Object catch (error) {
      debugPrint(
        'xue_hua_webview_android: document-start injection is unavailable: '
        '${error.toString().replaceAll(RegExp(r'[\r\n]+'), ' ')}',
      );
      _documentStartSupported = false;
    }
  }

  void _injectFallbackDocumentStartScripts(String url) {
    for (final UserScript userScript in _userScripts) {
      if (userScript.injectionTime == UserScriptInjectionTime.documentStart &&
          !_documentStartHandles.containsKey(userScript)) {
        unawaited(_webView.evaluateJavascript(userScript.source));
      }
    }
  }

  void _injectDocumentEndScripts(String url) {
    for (final UserScript userScript in _userScripts) {
  ...
  factory FileSelectorParams._fromFileChooserParams(
    ...
    switch (params.mode) {
      ...
      case android_webview.FileChooserMode.unknown:
        ...
        debugPrint(
          'xue_hua_webview_android: received an unknown file chooser mode; '
...
class AndroidNavigationDelegate extends PlatformNavigationDelegate {
  ...
  AndroidNavigationDelegate(PlatformNavigationDelegateCreationParams params)
    ...
    _webViewClient = android_webview.WebViewClient(
      onPageFinished: (_, android_webview.WebView webView, String url) {
        weakThis.target?._onPageFinishedInternal?.call(url);
      ...
      onPageStarted: (_, android_webview.WebView webView, String url) {
        weakThis.target?._onPageStartedInternal?.call(url);
  ...
  void Function(String url)? _onPageFinishedInternal;
  void Function(String url)? _onPageStartedInternal;
  ...
  /// Registers internal page callbacks used for user-script injection.
  void setInternalPageCallbacks({
    void Function(String url)? onPageStarted,
    void Function(String url)? onPageFinished,
  }) {
    _onPageStartedInternal = onPageStarted;
    _onPageFinishedInternal = onPageFinished;
  }
