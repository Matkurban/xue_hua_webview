...
@ConfigurePigeon(
  PigeonOptions(
    ...
    kotlinOut:
        'android/src/main/java/com/kurban/xue_hua_webview_android/AndroidWebkitLibrary.g.kt',
...
abstract class WebView extends View {
  ...
  /// Injects JavaScript to run as early as possible, before any other scripts.
  ///
  /// Returns a handle that can be passed to [removeDocumentStartJavaScript].
  /// Requires `WebViewFeature.DOCUMENT_START_SCRIPT`.
  String addDocumentStartJavaScript(
    String script,
    List<String> allowedOriginRules,
  );

  /// Removes a script previously added with [addDocumentStartJavaScript].
  void removeDocumentStartJavaScript(String scriptId);

  /// Creates a temporary WebView, clears its HTTP cache, and destroys it.
  @static
  void clearHttpCache();
}
...
@ProxyApi(
  kotlinOptions: KotlinProxyApiOptions(
    fullClassName:
        'com.kurban.xue_hua_webview_android.WebChromeClientProxyApi.WebChromeClientImpl',
  ),
)
...
@ProxyApi(
  kotlinOptions: KotlinProxyApiOptions(
    fullClassName: 'com.kurban.xue_hua_webview_android.FlutterAssetManager',
  ),
)
