...
import 'package:xue_hua_webview_android/src/android_webkit.g.dart'
    as android_webview;
import 'package:xue_hua_webview_android/src/android_webkit_constants.dart';
import 'package:xue_hua_webview_android/src/platform_views_service_proxy.dart';
...
void main() {
  ...
  group('AndroidWebViewController', () {
    ...
    test('addUserScript uses document-start when supported', () async {
      final mockWebView = MockWebView();
      when(
        mockWebView.addDocumentStartJavaScript(any, any),
      ).thenAnswer((_) async => 'script-1');
      final AndroidWebViewController controller = createControllerWithMocks(
        mockWebView: mockWebView,
        isWebViewFeatureSupported: (String feature) async =>
            feature == WebViewFeatureConstants.documentStartScript,
      );
    ...
    test('dispose destroys the native WebView', () async {
      final mockWebView = MockWebView();
      final AndroidWebViewController controller = createControllerWithMocks(
        mockWebView: mockWebView,
      );
